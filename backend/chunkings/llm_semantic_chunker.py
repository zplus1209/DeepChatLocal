...
_SYSTEM_PROMPT = (
    "Bạn là một chuyên gia phân đoạn văn bản theo chủ đề. "
    "Nhiệm vụ của bạn là phân tích một chuỗi các đoạn văn (chunks) và xác định "
    "những vị trí cần tách khi có sự thay đổi chủ đề rõ ràng.\n\n"

    "Mỗi đoạn được đánh dấu theo định dạng: "
    "<|start_chunk_X|> ... <|end_chunk_X|>, trong đó X là ID của đoạn.\n\n"

    "Mục tiêu của bạn:\n"
    "- Giữ các đoạn liên tiếp lại với nhau nếu chúng cùng một chủ đề.\n"
    "- Chỉ đề xuất tách khi có sự thay đổi chủ đề rõ ràng.\n"
    "- Không tách nếu nội dung vẫn còn liên quan chặt chẽ.\n\n"

    "QUY TẮC TRẢ LỜI (BẮT BUỘC TUÂN THỦ):\n"
    "1. Chỉ trả về DUY NHẤT một dòng.\n"
    "2. Định dạng chính xác: split_after: ID1, ID2, ID3\n"
    "3. Các ID phải là số nguyên.\n"
    "4. Các ID phải theo thứ tự tăng dần nghiêm ngặt.\n"
    "5. Không thêm giải thích, không thêm văn bản khác.\n"
)
...
class LLMSemanticChunker(TextSplitter):
    ...
    def _get_user_prompt(
        ...
        chunked_input: str,
        current_chunk: int,
        invalid_response: Optional[List[int]] = None,
    ) -> str:
        prompt = (
            "Dưới đây là văn bản đã được chia thành các đoạn:\n\n"
            f"{chunked_input}\n\n"
            "Nhiệm vụ của bạn:\n"
            "Xác định các vị trí cần tách khi có sự thay đổi chủ đề.\n\n"
            "QUY TẮC TRẢ LỜI (BẮT BUỘC):\n"
            f"- Phải có ít nhất một vị trí tách.\n"
            f"- Tất cả ID phải >= {current_chunk}.\n"
            "- Các ID phải theo thứ tự tăng dần nghiêm ngặt.\n"
            "- Định dạng trả lời: split_after: ID1, ID2\n"
            "- Không được thêm giải thích hay nội dung khác.\n"
        )

        if invalid_response:
            prompt += (
                f"\nCẢNH BÁO: Câu trả lời trước đó {invalid_response} là KHÔNG HỢP LỆ.\n"
                "Không được lặp lại câu trả lời đó.\n"
                "Hãy đảm bảo tuân thủ đầy đủ tất cả các quy tắc định dạng.\n"
            )
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        pbar = tqdm(total=len(chunks), desc="LLMSemanticChunker", disable=not self.disable)
        with pbar:
            while True and not short_cut:
                ...
                for i in range(current_chunk, len(chunks)):
                    token_count += _default_length_function(chunks[i])
                    chunked_input += f"<|start_chunk_{i+1}|>{chunks[i]}<|end_chunk_{i+1}|>"
                    if token_count > 800:
                        break

                prompt = self._get_user_prompt(chunked_input, current_chunk)
                while True:
                    result_string = self.llm.invoke(prompt=prompt, system_prompt=_SYSTEM_PROMPT)
                    ...
                    if not (numbers != sorted(numbers) or any(number < current_chunk for number in numbers)):
                        ...
                        prompt = self._get_user_prompt(chunked_input, current_chunk, numbers)
